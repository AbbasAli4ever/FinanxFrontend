...
const InventoryStatsCard: React.FC<Props> = ({ data, loading }) => (
  <div className="rounded-lg border border-gray-200 bg-white p-4 shadow-sm dark:border-gray-700 dark:bg-gray-900">
    <div className="mb-4 flex items-center gap-2">
      <div className="flex h-8 w-8 items-center justify-center rounded bg-sky-50 dark:bg-sky-900/20">
      ...
      <h3 className="text-[14px] font-semibold text-gray-900 dark:text-white">Inventory</h3>
    </div>
      ...
      <>
        ...
        <div className="mb-4 grid grid-cols-2 gap-2">
          <div className="rounded bg-gray-50 p-3 dark:bg-gray-800/60">
            ...
            <p className="text-[18px] font-bold text-gray-900 dark:text-white">{data.summary.totalTrackedProducts}</p>
          </div>
          <div className="rounded bg-sky-50/60 p-3 dark:bg-sky-900/20">
            <p className="text-[10px] uppercase tracking-wide text-sky-600 dark:text-sky-400">Value</p>
            <p className="text-[18px] font-bold text-sky-700 dark:text-sky-300">{fmt(data.summary.totalInventoryValue)}</p>
          </div>
          <div className="rounded bg-warning-50/60 p-3 dark:bg-warning-900/20">
            ...
            <p className="text-[18px] font-bold text-warning-700 dark:text-warning-300">{data.summary.lowStockCount}</p>
          </div>
          <div className="rounded bg-error-50/60 p-3 dark:bg-error-900/20">
            ...
            <p className="text-[18px] font-bold text-error-700 dark:text-error-300">{data.summary.outOfStockCount}</p>
          </div>
        </div>
          ...
          <div className="mb-4">
            ...
            <div className="space-y-1.5">
              {data.lowStockAlerts.slice(0, 4).map((item) => (
                <div
                  key={item.productId}
                  className="flex items-center justify-between rounded border border-warning-100 bg-warning-50/40 px-3 py-2 dark:border-warning-800/30 dark:bg-warning-900/10"
                >
                  <div>
            ...
            <div className="space-y-1.5">
              {data.topSellingProducts.slice(0, 4).map((item, i) => (
                <div key={item.productId} className="flex items-center gap-2 text-xs">
                  <span className="flex h-5 w-5 flex-shrink-0 items-center justify-center rounded bg-success-100 text-[10px] font-bold text-success-700 dark:bg-success-900/30 dark:text-success-300">
                    {i + 1}
                  </span>
