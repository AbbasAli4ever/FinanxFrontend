...
import type { PaymentTerms, DiscountType } from "@/types/invoices";
...
const CreateInvoiceModal: React.FC<CreateInvoiceModalProps> = ({
  ...
  // Load next invoice number when modal opens
  useEffect(() => {
    if (isOpen && token) {
      ...
      prev.map((item, i) =>
        i === rowIndex
          ? {
              ...
              taxPercent: product.taxable ? (product as ProductListItem & { taxRate?: number | null }).taxRate ?? 0 : 0,
            }
          : item
      )
    );
  ...
  return (
    <Modal isOpen={isOpen} onClose={handleClose} className="max-w-4xl p-6 lg:p-8">
      ...
      <form onSubmit={handleSubmit}>
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            ...
            <div>
              <Label htmlFor="ci-number">Invoice Number</Label>
              <Input
                id="ci-number"
                value={invoiceNumber}
            ...
            <div>
              <Label htmlFor="ci-ref">Reference Number</Label>
              <Input
                id="ci-ref"
            ...
            <div className="w-full max-w-sm space-y-2">
              ...
              <div className="flex items-center gap-2">
                ...
                {totals.invoiceDiscount > 0 && (
                  <span className="ml-auto text-sm text-error-500">
                    -{formatCurrency(totals.invoiceDiscount)}
                  </span>
                )}
              </div>
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            <div>
              <Label htmlFor="ci-account">Deposit Account</Label>
              <select
                id="ci-account"
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-500 dark:text-gray-400">Auto-clone this invoice on a schedule</p>
                </div>
              </div>
              <button
                type="button"
              ...
              <div className="mt-4 grid grid-cols-1 gap-4 sm:grid-cols-2">
                ...
                <div className="col-span-full rounded-lg border border-brand-100 bg-brand-50/60 px-3 py-2.5 dark:border-brand-800 dark:bg-brand-900/10">
                  <p className="text-xs text-brand-700 dark:text-brand-300">
                    <strong>How it works:</strong> A clone of this invoice will be auto-generated as a DRAFT each time it&apos;s sent (via the &apos;Send&apos; action). The next recurring date is calculated from the invoice date + frequency.
                  </p>
                </div>
              </div>
            )}
      ...
      {/* Product search dropdown — portalled to body to escape overflow clipping */}
