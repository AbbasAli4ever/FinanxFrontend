...
const CreateJournalEntryModal: React.FC<CreateJournalEntryModalProps> = ({
  ...
  useEffect(() => {
    ...
    if (entryType === "ADJUSTING") {
      // Allow auto-reversing but don't force it
    } else {
      setIsAutoReversing(false);
      setReversalDate("");
    }
  }, [entryType]);
  ...
  const updateLine = (key: string, field: keyof EntryLine, value: string | number) => {
    setLines((prev) =>
      prev.map((l) => {
        ...
        // When user enters debit > 0, clear credit (and vice versa)
      ...
      <div className="max-h-[70vh] overflow-y-auto pr-2 space-y-6">
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
          ...
          <div>
            ...
            <Input type="text" value={referenceNumber} onChange={(e) => setReferenceNumber(e.target.value)} placeholder="Optional" />
          </div>
        </div>
