...
import rolesService, { UpdateRoleRequest, Permission, Role } from "@/services/rolesService";
...
export default function EditRolePage() {
  ...
  useEffect(() => {
    if (!hasAccess || !roleId || !token || permissionsLoading) return;
    if (!canManageRoles) {
    ...
    const loadData = async () => {
      ...
      try {
        const [roleData, perms] = await Promise.all([
          rolesService.getRole(roleId, token),
          permissionsService.getAllPermissions(token),
        ]);
    ...
    loadData();
  }, [hasAccess, roleId, token, permissionsLoading, canManageRoles]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await rolesService.updateRole(roleId, payload, token);

      setAlert({
        variant: "success",
        title: "Role updated",
        message: `The role "${formData.name}" has been updated successfully.`,
      });
  ...
  if (!isReady || permissionsLoading || loadingData) {
    return (
      <div className="rounded-2xl border border-dashed border-gray-300 bg-white/80 p-6 text-center text-sm text-gray-600 dark:border-gray-700 dark:bg-gray-900/60">
        <div className="flex items-center justify-center gap-2">
          ...
          <span>{!isReady ? "Checking authentication..." : "Loading role data..."}</span>
        </div>
      </div>
    );
  }
      ...
      <div className="space-y-6 p-4 sm:p-8">
        ...
        <div className="rounded-2xl border border-warning-200 bg-warning-50 p-6 text-center dark:border-warning-800 dark:bg-warning-900/20">
          ...
          <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
            You do not have permission to edit roles. Contact your administrator for access.
          </p>
  ...
  if (role.isSystemRole) {
    return (
      <div className="space-y-6 p-4 sm:p-8">
        <div className="rounded-2xl border border-yellow-200 bg-yellow-50 p-6 text-center dark:border-yellow-800 dark:bg-yellow-900/20">
          <p className="text-sm font-medium text-yellow-800 dark:text-yellow-400">
            System roles cannot be modified
          </p>
          <p className="mt-2 text-xs text-yellow-700 dark:text-yellow-500">
            The role &quot;{role.name}&quot; is a system role and cannot be edited.
          </p>
          <Button variant="outline" className="mt-4" onClick={() => router.push("/roles")}>
            Back to Roles
          </Button>
