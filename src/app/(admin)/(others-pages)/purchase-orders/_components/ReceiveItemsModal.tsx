...
const ReceiveItemsModal: React.FC<Props> = ({ isOpen, po, onClose, onReceived }) => {
  const { token } = useAuth();
  ...
  useEffect(() => {
    if (!po || !isOpen) return;
    const receivable: ReceiveItem[] = po.lineItems
      .filter((line: POLineItem) => line.quantityReceived < line.quantity)
      .map((line: POLineItem) => ({
        lineItemId: line.id,
  ...
  const setToReceive = (lineItemId: string, value: number) => {
    setItems((prev) =>
      prev.map((item) =>
        item.lineItemId === lineItemId
          ? { ...item, toReceive: Math.max(0, Math.min(value, item.remaining)) }
          : item
      )
    );
  };
  ...
  const handleSubmit = async () => {
    if (!token || !po) return;
    const itemsToSend = items.filter((item) => item.toReceive > 0);
    if (itemsToSend.length === 0) {
      setError("Please enter a quantity to receive for at least one line item.");
      return;
    }
    setLoading(true);
    setError("");
    try {
      await purchaseOrdersService.receiveItems(po.id, {
        receivedDate: receivedDate || undefined,
        items: itemsToSend.map((item) => ({
          lineItemId: item.lineItemId,
          quantityReceived: item.toReceive,
        })),
      }, token);
      onReceived();
      onClose();
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} className="max-w-2xl p-6">
      <div className="flex flex-col gap-5">
        {/* Header */}
        <div>
          <h2 className="text-lg font-bold text-gray-900 dark:text-white">Receive Items</h2>
          <p className="mt-0.5 text-sm text-gray-500 dark:text-gray-400">
            {po.poNumber} — {po.vendor.displayName}
          </p>
        </div>
        ...
        <div className="flex items-start gap-2 rounded-lg bg-blue-50 p-3 dark:bg-blue-900/10">
          ...
          <p className="text-xs text-blue-700 dark:text-blue-300">
            Quantities are <strong>additive</strong> — values entered here will be added to existing received amounts per line.
          </p>
        </div>
        ...
        {items.length === 0 ? (
          <div className="rounded-xl border border-gray-200 py-10 text-center dark:border-gray-800">
            <p className="text-sm font-medium text-gray-500">All items have been fully received.</p>
          </div>
        ) : (
            ...
            <table className="min-w-full text-sm">
              <thead>
                <tr className="border-b border-gray-200 bg-gray-50 dark:border-gray-800 dark:bg-gray-800/40">
                  ...
                  <th className="px-4 py-2.5 text-center text-xs font-medium text-gray-500">Received</th>
                  <th className="px-4 py-2.5 text-center text-xs font-medium text-gray-500">Remaining</th>
                  <th className="px-4 py-2.5 text-center text-xs font-medium text-gray-500">Qty to Receive</th>
              ...
              <tbody className="divide-y divide-gray-100 dark:divide-gray-800">
                {items.map((item) => {
                  const progressAfter = Math.min(
                    ((item.quantityReceived + item.toReceive) / item.quantity) * 100,
                    100
                  );
                  return (
                    <tr key={item.lineItemId}>
                      ...
                      <td className="px-4 py-3 text-center">
                        <input
                          ...
                          value={item.toReceive === 0 ? "" : item.toReceive}
                          onChange={(e) => setToReceive(item.lineItemId, parseInt(e.target.value) || 0)}
                          placeholder="0"
            ...
            {totalToReceive > 0 && (
              <div className="border-t border-gray-200 bg-gray-50/70 px-4 py-2.5 dark:border-gray-800 dark:bg-gray-800/30">
                <p className="text-right text-xs font-medium text-gray-600 dark:text-gray-400">
                  Total units to receive: <span className="text-gray-900 dark:text-white">{totalToReceive}</span>
                </p>
              </div>
            )}
          </div>
        ...
        <div className="flex justify-end gap-3 border-t border-gray-200 pt-4 dark:border-gray-800">
          ...
          <Button onClick={handleSubmit} disabled={loading || items.length === 0}>
            {loading ? "Recording..." : `Record Receipt${totalToReceive > 0 ? ` (${totalToReceive} units)` : ""}`}
          </Button>
        </div>
      </div>
    </Modal>
