...
import type { PurchaseOrder, UpdatePORequest, CreatePOLineItemRequest, DiscountType } from "@/types/purchaseOrders";
...
const EditPurchaseOrderModal: React.FC<Props> = ({
  isOpen, poId, onClose, onUpdated, vendors, accounts,
}) => {
  const { token } = useAuth();
  ...
  useEffect(() => {
    ...
    purchaseOrdersService.getPurchaseOrder(poId, token)
      .then((data) => {
        ...
        setNotes(data.notes ?? ""); setMemo(data.memo ?? ""); setVendorMessage(data.vendorMessage ?? "");
        setShippingLine1(data.shippingAddressLine1 ?? "");
        setShippingLine2(data.shippingAddressLine2 ?? "");
        setShippingCity(data.shippingCity ?? ""); setShippingState(data.shippingState ?? "");
        setShippingPostalCode(data.shippingPostalCode ?? ""); setShippingCountry(data.shippingCountry ?? "");
        const hasShipping = !!(data.shippingAddressLine1 || data.shippingCity);
  ...
  const handleClose = () => {
    setPo(null); setError(""); setLineItems([newRow()]);
    onClose();
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await purchaseOrdersService.updatePurchaseOrder(poId, payload, token);
      ...
      {fetchLoading ? (
        ...
        <div className="flex flex-col gap-6">
          <div>
            ...
            {po && <p className="mt-0.5 text-sm text-gray-500 dark:text-gray-400">{po.poNumber} — Draft only</p>}
          </div>
                ...
                <tbody className="divide-y divide-gray-100 dark:divide-gray-800">
                  {lineItems.map((row, idx) => (
                    <tr key={row.id}>
                      ...
                      <td className="px-2 py-2">
                        <select value={row.expenseAccountId} onChange={(e) => updateRow(row.id, "expenseAccountId", e.target.value)}
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
            <div>
              ...
              <TextArea value={notes} onChange={setNotes} placeholder="Notes..." rows={3} />
            </div>
            <div>
            ...
            <div>
              ...
              <TextArea value={vendorMessage} onChange={setVendorMessage} placeholder="Vendor message..." rows={3} />
            </div>
          </div>
