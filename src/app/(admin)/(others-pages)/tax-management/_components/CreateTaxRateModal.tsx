...
import type { TaxType } from "@/types/taxes";
...
const CreateTaxRateModal: React.FC<Props> = ({ isOpen, onClose, onCreated }) => {
  const { token } = useAuth();
  ...
  const handleClose = () => {
    setForm({ name: "", code: "", description: "", rate: "", taxType: "BOTH", isCompound: false });
    setError(null);
    onClose();
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setError(null);
    try {
      await taxesService.createRate(token, {
        name: form.name.trim(),
  ...
  return (
    <Modal isOpen={isOpen} onClose={handleClose} className="max-w-lg w-full">
      <div className="p-6">
        <div className="mb-6">
          <h2 className="text-lg font-bold text-gray-900 dark:text-white">Create Tax Rate</h2>
          <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
            Define a new tax rate for use on invoices, bills, and expenses.
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <div>
              <Label htmlFor="rate-name">Name <span className="text-error-500">*</span></Label>
              <Input
                id="rate-name"
                type="text"
                placeholder="e.g. GST"
            ...
            <div>
              <Label htmlFor="rate-code">Code <span className="text-error-500">*</span></Label>
              <Input
                id="rate-code"
                type="text"
                placeholder="e.g. GST"
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <Label htmlFor="rate-percent">Rate (%) <span className="text-error-500">*</span></Label>
              <Input
                id="rate-percent"
                type="number"
                placeholder="e.g. 5"
          ...
          <div className="flex items-center gap-3 rounded-lg border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800/50 px-4 py-3">
            ...
            />
            <div>
              <label htmlFor="rate-compound" className="text-sm font-medium text-gray-800 dark:text-gray-200 cursor-pointer">
                Compound Tax
              </label>
              <p className="text-xs text-gray-500 dark:text-gray-400">
                Applied on top of base amount + any previous taxes
              </p>
            </div>
          </div>
