...
import type { DebitNote, CreateDebitNoteLineItemRequest } from "@/types/debitNotes";
...
const EditDebitNoteModal: React.FC<EditDebitNoteModalProps> = ({
  isOpen, debitNoteId, onClose, onUpdated, vendors,
}) => {
  const { token } = useAuth();
  ...
  const populateForm = useCallback((dn: DebitNote) => {
    setVendorId(dn.vendor.id);
    setBillId(dn.bill?.id ?? "");
    setDebitNoteNumber(dn.debitNoteNumber);
    setReferenceNumber(dn.referenceNumber ?? "");
    setDebitNoteDate(dn.debitNoteDate.split("T")[0]);
    setDiscountType((dn.discountType as "" | "PERCENTAGE" | "FIXED") ?? "");
    setDiscountValue(dn.discountValue ?? "");
    setReason(dn.reason ?? "");
    setNotes(dn.notes ?? "");
    setLines(
      dn.lineItems.length > 0
        ? dn.lineItems.map((l) => ({
            ...
            expenseAccountId: l.expenseAccount?.id ?? "",
  ...
  }, [isOpen, debitNoteId, token, populateForm]);
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} className="max-w-4xl p-6 lg:p-8">
      <div className="flex flex-col gap-6">
        <div>
          <h2 className="text-xl font-semibold text-gray-900 dark:text-white">Edit Debit Note</h2>
          <p className="mt-0.5 text-sm text-gray-500 dark:text-gray-400">Modify this draft debit note</p>
        </div>
        ...
        {fetching ? (
          ...
          <>
                  ...
                  <tbody className="divide-y divide-gray-100 dark:divide-gray-800">
                    {lines.map((line, idx) => (
                      <tr key={line.key}>
                        ...
                        <td className="px-3 py-2 min-w-[180px]"><input type="text" value={line.description} onChange={(e) => updateLine(idx, "description", e.target.value)} placeholder="Description" className="w-full rounded border border-gray-300 px-2 py-1.5 text-xs dark:border-gray-700 dark:bg-gray-900 dark:text-white/90" /></td>
                        <td className="px-3 py-2 w-16"><input type="number" value={line.quantity} onChange={(e) => updateLine(idx, "quantity", parseFloat(e.target.value) || 0)} min="0.001" step={1} className="w-full rounded border border-gray-300 px-2 py-1.5 text-right text-xs dark:border-gray-700 dark:bg-gray-900 dark:text-white/90" /></td>
                        <td className="px-3 py-2 w-24"><input type="number" value={line.unitPrice} onChange={(e) => updateLine(idx, "unitPrice", parseFloat(e.target.value) || 0)} min="0" step={0.01} className="w-full rounded border border-gray-300 px-2 py-1.5 text-right text-xs dark:border-gray-700 dark:bg-gray-900 dark:text-white/90" /></td>
                        <td className="px-3 py-2 w-16"><input type="number" value={line.discountPercent} onChange={(e) => updateLine(idx, "discountPercent", parseFloat(e.target.value) || 0)} min="0" max="100" step={0.1} className="w-full rounded border border-gray-300 px-2 py-1.5 text-right text-xs dark:border-gray-700 dark:bg-gray-900 dark:text-white/90" /></td>
                        <td className="px-3 py-2 w-16"><input type="number" value={line.taxPercent} onChange={(e) => updateLine(idx, "taxPercent", parseFloat(e.target.value) || 0)} min="0" max="100" step={0.1} className="w-full rounded border border-gray-300 px-2 py-1.5 text-right text-xs dark:border-gray-700 dark:bg-gray-900 dark:text-white/90" /></td>
            ...
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
              <div><Label>Reason</Label><TextArea placeholder="Reason for debit note..." value={reason} onChange={setReason} rows={3} /></div>
              <div><Label>Notes</Label><TextArea placeholder="Internal notes..." value={notes} onChange={setNotes} rows={3} /></div>
            </div>
          </>
        )}
...
export default EditDebitNoteModal;
