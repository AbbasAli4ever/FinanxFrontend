...
function bucketAmt(customer: ArAgingCustomer, key: BucketKey): number {
  return customer[key] ?? 0;
}

// ── Invoice sub-rows ──────────────────────────────────────────
const InvoiceRows: React.FC<{ invoices: AgingInvoice[] }> = ({ invoices }) => (
  <tr>
        ...
        <table className="w-full text-xs">
          <thead>
            <tr className="text-gray-400 dark:text-gray-500">
              <th className="px-8 py-2 text-left font-medium">Invoice #</th>
          ...
          <tbody className="divide-y divide-gray-100 dark:divide-gray-700">
            {invoices.map((inv) => {
              const bc = BUCKET_COLORS[inv.bucket];
              return (
                <tr key={inv.invoiceId} className="bg-white dark:bg-gray-900/20">
                  <td className="px-8 py-2 font-mono font-semibold text-brand-600 dark:text-brand-400">
                    {inv.invoiceNumber}
                  </td>
                  <td className="px-3 py-2 text-gray-600 dark:text-gray-400">{shortDate(inv.invoiceDate)}</td>
                  <td className="px-3 py-2 text-gray-600 dark:text-gray-400">{shortDate(inv.dueDate)}</td>
...
const ArAgingReport: React.FC = () => {
  ...
  const fetchReport = useCallback(async () => {
    ...
    setError("");
    try {
      const result = await reportsService.getArAging(
          ...
          <div className="flex items-center justify-between border-b border-gray-200 p-5 dark:border-gray-800">
            <div>
              ...
              <p className="text-sm text-gray-500 dark:text-gray-400">
                As of {shortDate(data.asOfDate)} · {data.customers.length} customer{data.customers.length !== 1 ? "s" : ""}
              </p>
            </div>
            <span className="rounded-xl bg-brand-50 px-4 py-2 text-sm font-bold text-brand-700 dark:bg-brand-900/20 dark:text-brand-300">
              Total AR: {formatCurrency(data.totals.total)}
            </span>
          </div>
              ...
              <table className="w-full min-w-[720px]">
                ...
                <tbody className="divide-y divide-gray-100 dark:divide-gray-700">
                  {data.customers.map((customer) => {
                    const isOpen = expandedRows.has(customer.customerId);
                    return (
                          ...
                          <td className="px-5 py-3">
                            <div className="flex items-center gap-2">
                              ...
                              <span className="rounded-full bg-gray-100 px-1.5 py-0.5 text-[10px] text-gray-500 dark:bg-gray-700 dark:text-gray-400">
                                {customer.invoices.length} inv
                              </span>
                            </div>
                          </td>
                          {BUCKETS.map((b) => {
                            const amt = bucketAmt(customer, b);
