...
const CreateEstimateModal: React.FC<CreateEstimateModalProps> = ({
  isOpen, customers, accounts, onClose, onCreated,
}) => {
  const { token } = useAuth();
  ...
  const [expirationDate, setExpirationDate] = useState(addDays(30));
  ...
  const [lineItems, setLineItems] = useState<LineItem[]>([
    { key: newKey(), productId: "", description: "", quantity: 1, unitPrice: 0, discountPercent: 0, taxPercent: 0 },
  ]);
  ...
  const reset = useCallback(() => {
    setCustomerId(""); setEstimateDate(todayStr()); setExpirationDate(addDays(30));
    setPaymentTerms(""); setDiscountType("PERCENTAGE"); setDiscountValue("");
    setDepositAccountId(""); setNotes(""); setTermsAndConditions(""); setCustomerMessage("");
    setLineItems([{ key: newKey(), productId: "", description: "", quantity: 1, unitPrice: 0, discountPercent: 0, taxPercent: 0 }]);
    setError("");
  }, []);
  ...
  const addLine = () => {
    setLineItems((prev) => [...prev, { key: newKey(), productId: "", description: "", quantity: 1, unitPrice: 0, discountPercent: 0, taxPercent: 0 }]);
  };
  ...
  const handleSubmit = async () => {
    ...
    if (!estimateDate) { setError("Please enter an estimate date."); return; }
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} className="max-w-4xl p-6 lg:p-8">
      <div className="flex flex-col gap-6">
        {/* Header */}
        <div>
          <h2 className="text-xl font-bold text-gray-900 dark:text-white">Create Estimate</h2>
          <p className="mt-0.5 text-sm text-gray-500 dark:text-gray-400">Build a quote to send to your customer</p>
        </div>
                    ...
                    <tr key={line.key}>
                      <td className="px-3 py-2">
                        <select
                          ...
                          className="w-full rounded border border-gray-300 px-2 py-1.5 text-xs dark:border-gray-700 dark:bg-gray-900 dark:text-white/90"
                        >
                          <option value="">None</option>
                      ...
                      <td className="px-3 py-2">
                        <input type="number" value={line.quantity} onChange={(e) => updateLine(line.key, "quantity", parseFloat(e.target.value) || 0)} min="0.001" step={1} className="w-full rounded border border-gray-300 px-2 py-1.5 text-right text-xs dark:border-gray-700 dark:bg-gray-900 dark:text-white/90" />
                      </td>
                      <td className="px-3 py-2">
                        <input type="number" value={line.unitPrice} onChange={(e) => updateLine(line.key, "unitPrice", parseFloat(e.target.value) || 0)} min="0" step={0.01} className="w-full rounded border border-gray-300 px-2 py-1.5 text-right text-xs dark:border-gray-700 dark:bg-gray-900 dark:text-white/90" />
                      </td>
                      <td className="px-3 py-2">
                        <input type="number" value={line.discountPercent} onChange={(e) => updateLine(line.key, "discountPercent", parseFloat(e.target.value) || 0)} min="0" max="100" step={0.1} className="w-full rounded border border-gray-300 px-2 py-1.5 text-right text-xs dark:border-gray-700 dark:bg-gray-900 dark:text-white/90" />
                      </td>
                      <td className="px-3 py-2">
                        <input type="number" value={line.taxPercent} onChange={(e) => updateLine(line.key, "taxPercent", parseFloat(e.target.value) || 0)} min="0" max="100" step={0.1} className="w-full rounded border border-gray-300 px-2 py-1.5 text-right text-xs dark:border-gray-700 dark:bg-gray-900 dark:text-white/90" />
                      </td>
                      <td className="px-3 py-2 text-right text-xs font-medium tabular-nums text-gray-900 dark:text-white/90">
              ...
              <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <input
                    ...
                    min="0"
                    step={0.01}
                    className="w-24 rounded border border-gray-300 px-2 py-1 text-right text-xs dark:border-gray-700 dark:bg-gray-900 dark:text-white/90"
                  />
                </div>
                {/* Summary */}
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
          <div>
            ...
            <TextArea placeholder="Message shown to customer..." value={customerMessage} onChange={setCustomerMessage} rows={3} />
          </div>
          <div>
            <Label>Terms & Conditions (optional)</Label>
            <TextArea placeholder="Payment terms, warranty info..." value={termsAndConditions} onChange={setTermsAndConditions} rows={3} />
          </div>
          <div>
