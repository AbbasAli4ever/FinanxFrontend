...
const EditExpenseModal: React.FC<EditExpenseModalProps> = ({
  ...
  const populateForm = useCallback((expense: Expense) => {
    setExpenseNumber(expense.expenseNumber);
    setExpenseDate(expense.expenseDate);
    setExpenseAccountId(expense.expenseAccount.id);
    setAmount(expense.amount);
    setDescription(expense.description || "");
    setReferenceNumber(expense.referenceNumber || "");
    setNotes(expense.notes || "");
    setVendorId(expense.vendor?.id || "");
    setCategoryId(expense.category?.id || "");
    setPaymentMethod(expense.paymentMethod || "");
    setPaymentAccountId(expense.paymentAccount?.id || "");
    setTaxPercent(expense.taxPercent);
    setIsTaxDeductible(expense.isTaxDeductible);
    setIsBillable(expense.isBillable);
    setBillableCustomerId(expense.billableCustomer?.id || "");
    setMarkupPercent(expense.markupPercent || 0);
    setIsReimbursable(expense.isReimbursable);
    setIsMileage(expense.isMileage);
    setMileageDistance(expense.mileageDistance || 0);
    setMileageRate(expense.mileageRate || 0.655);
  ...
  }, [isOpen, expenseId, token, populateForm]);
  ...
  const taxAmount = useMemo(() => {
    if (isSplit) return lineItems.reduce((sum, li) => sum + li.amount * (li.taxPercent / 100), 0);
  ...
  const markedUpAmount = useMemo(() => {
    if (isBillable && markupPercent > 0) return totalAmount * (1 + markupPercent / 100);
    return null;
  }, [isBillable, markupPercent, totalAmount]);

  // Line item helpers
  const addLineItem = () => {
    setLineItems((prev) => [...prev, { key: generateKey(), expenseAccountId: "", description: "", amount: 0, taxPercent: 0 }]);
  };
      ...
      {loadingData ? (
        ...
        <>
          <div className="max-h-[70vh] overflow-y-auto pr-2 space-y-6">
            ...
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
              <div>
                <Label>Expense Account <span className="text-error-500">*</span></Label>
                <select value={expenseAccountId} onChange={(e) => setExpenseAccountId(e.target.value)} className={selectClasses}>
              ...
              <div>
                <Label>Vendor</Label>
                <select value={vendorId} onChange={(e) => setVendorId(e.target.value)} className={selectClasses}>
            ...
            <div>
              ...
              <TextArea value={description} onChange={(val) => setDescription(val)} placeholder="What is this expense for?" rows={2} />
            </div>
            ...
            {!isMileage && !isSplit && (
              <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
                <div>
                  ...
                  <Input type="number" value={amount.toString()} onChange={(e) => setAmount(parseFloat(e.target.value) || 0)} step={0.01} min="0" />
                </div>
                <div>
                  <Label>Tax %</Label>
                  <Input type="number" value={taxPercent.toString()} onChange={(e) => setTaxPercent(parseFloat(e.target.value) || 0)} step={0.01} min="0" max="100" />
                </div>
            ...
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
              ...
              <div>
                ...
                <select value={paymentAccountId} onChange={(e) => setPaymentAccountId(e.target.value)} className={selectClasses}>
                  <option value="">Select account</option>
                  {accounts.map((a) => (<option key={a.id} value={a.id}>{a.accountNumber} — {a.name}</option>))}
                </select>
              </div>
            ...
            <div className="rounded-xl border border-gray-200 p-4 dark:border-gray-700">
              <h3 className="mb-3 text-sm font-semibold text-gray-900 dark:text-white">Options</h3>
              <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-6">
                {[
                  ...
                  <label key={toggle.label} className={`flex cursor-pointer items-center gap-2 rounded-lg border px-3 py-2 text-sm transition ${toggle.checked ? "border-brand-300 bg-brand-50 text-brand-700 dark:border-brand-700 dark:bg-brand-900/20 dark:text-brand-300" : "border-gray-200 bg-white text-gray-600 hover:bg-gray-50 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-400 dark:hover:bg-gray-800"}`}>
                    <input type="checkbox" checked={toggle.checked} onChange={(e) => toggle.set(e.target.checked)} className="h-3.5 w-3.5 rounded border-gray-300 text-brand-500 focus:ring-brand-500" />
                    {toggle.label}
                  </label>
                ))}
              </div>
            ...
            {isBillable && (
              <div className="rounded-xl border border-blue-200 bg-blue-50/30 p-4 dark:border-blue-800 dark:bg-blue-900/10">
                <h4 className="mb-3 text-sm font-semibold text-blue-700 dark:text-blue-300">Billable Details</h4>
                <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                  <div>
                    <Label>Customer</Label>
                    <select value={billableCustomerId} onChange={(e) => setBillableCustomerId(e.target.value)} className={selectClasses}>
                  ...
                  <div>
                    <Label>Markup %</Label>
                    <Input type="number" value={markupPercent.toString()} onChange={(e) => setMarkupPercent(parseFloat(e.target.value) || 0)} step={0.01} min="0" />
                    {markedUpAmount !== null && (
                      <p className="mt-1 text-xs text-blue-600 dark:text-blue-400">Billable amount: {formatCurrency(markedUpAmount)}</p>
                    )}
                  </div>
                </div>
              </div>
            ...
            {isMileage && (
              <div className="rounded-xl border border-orange-200 bg-orange-50/30 p-4 dark:border-orange-800 dark:bg-orange-900/10">
                <h4 className="mb-3 text-sm font-semibold text-orange-700 dark:text-orange-300">Mileage Details</h4>
                <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
                  <div>
                    <Label>Distance (miles)</Label>
                    <Input type="number" value={mileageDistance.toString()} onChange={(e) => setMileageDistance(parseFloat(e.target.value) || 0)} step={0.1} min="0" />
                  </div>
                  <div>
                    <Label>Rate ($/mile)</Label>
                    <Input type="number" value={mileageRate.toString()} onChange={(e) => setMileageRate(parseFloat(e.target.value) || 0)} step={0.001} min="0" />
                  </div>
                  <div className="flex items-end">
                    <div className="rounded-lg bg-orange-50 px-4 py-2.5 dark:bg-orange-900/20">
                      <p className="text-xs text-orange-500 dark:text-orange-400">Calculated</p>
                      <p className="text-lg font-bold text-orange-700 dark:text-orange-200">{formatCurrency(computedAmount)}</p>
                    </div>
                  </div>
                </div>
              </div>
            ...
            {isSplit && (
              <div className="rounded-xl border border-gray-200 p-4 dark:border-gray-700">
                <div className="mb-3 flex items-center justify-between">
                  <h4 className="text-sm font-semibold text-gray-900 dark:text-white">Split Line Items</h4>
                  <Button variant="outline" size="sm" onClick={addLineItem}>+ Add Line</Button>
                </div>
                <div className="space-y-3">
                  {lineItems.map((li) => (
                    <div key={li.key} className="grid grid-cols-12 items-end gap-2 rounded-lg border border-gray-100 bg-gray-50/50 p-3 dark:border-gray-700 dark:bg-gray-800/30">
                      <div className="col-span-4">
                        <label className="mb-1 block text-xs text-gray-500 dark:text-gray-400">Account</label>
                        <select value={li.expenseAccountId} onChange={(e) => updateLineItem(li.key, "expenseAccountId", e.target.value)} className="h-9 w-full appearance-none rounded-lg border border-gray-300 px-2 text-xs shadow-theme-xs focus:border-brand-300 focus:outline-hidden focus:ring-3 focus:ring-brand-500/10 dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:focus:border-brand-800">
                      ...
                      <div className="col-span-3">
                        ...
                        <input type="text" value={li.description} onChange={(e) => updateLineItem(li.key, "description", e.target.value)} className="h-9 w-full rounded-lg border border-gray-300 px-2 text-xs shadow-theme-xs focus:border-brand-300 focus:outline-hidden focus:ring-3 focus:ring-brand-500/10 dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:focus:border-brand-800" placeholder="Description" />
                      </div>
                      <div className="col-span-2">
                        <label className="mb-1 block text-xs text-gray-500 dark:text-gray-400">Amount</label>
                        <input type="number" value={li.amount || ""} onChange={(e) => updateLineItem(li.key, "amount", parseFloat(e.target.value) || 0)} step={0.01} min={0} className="h-9 w-full rounded-lg border border-gray-300 px-2 text-xs text-right tabular-nums shadow-theme-xs focus:border-brand-300 focus:outline-hidden focus:ring-3 focus:ring-brand-500/10 dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:focus:border-brand-800" />
                      </div>
                      <div className="col-span-2">
                        <label className="mb-1 block text-xs text-gray-500 dark:text-gray-400">Tax %</label>
                        <input type="number" value={li.taxPercent || ""} onChange={(e) => updateLineItem(li.key, "taxPercent", parseFloat(e.target.value) || 0)} step={0.01} min={0} className="h-9 w-full rounded-lg border border-gray-300 px-2 text-xs text-right tabular-nums shadow-theme-xs focus:border-brand-300 focus:outline-hidden focus:ring-3 focus:ring-brand-500/10 dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:focus:border-brand-800" />
                      </div>
                      <div className="col-span-1 flex justify-end">
                        <button type="button" onClick={() => removeLineItem(li.key)} disabled={lineItems.length <= 2} className="rounded-lg p-1.5 text-gray-400 transition hover:bg-red-50 hover:text-red-500 disabled:cursor-not-allowed disabled:opacity-30 dark:hover:bg-red-900/20 dark:hover:text-red-400" title="Remove">
            ...
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
              ...
              <div>
                ...
                <Input type="text" value={receiptFileName} onChange={(e) => setReceiptFileName(e.target.value)} placeholder="receipt.pdf" />
              </div>
            </div>
            ...
            <div>
              <Label>Notes</Label>
              <TextArea value={notes} onChange={(val) => setNotes(val)} placeholder="Additional notes..." rows={2} />
            </div>
          </div>
