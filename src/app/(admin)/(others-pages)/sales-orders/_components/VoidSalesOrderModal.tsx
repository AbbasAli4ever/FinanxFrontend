...
const VoidSalesOrderModal: React.FC<Props> = ({ isOpen, soId, soNumber, onClose, onVoided }) => {
  const { token } = useAuth();
  ...
  const handleVoid = async () => {
    ...
    try {
      await salesOrdersService.voidSalesOrder(soId, { reason: reason.trim() || undefined }, token);
      onVoided(); onClose(); setReason("");
    } catch (err) {
      ...
      <div className="flex flex-col gap-5">
        <div className="flex items-start gap-4">
          ...
          <div className="flex-1">
            <h3 className="text-base font-semibold text-gray-900 dark:text-white">Void Sales Order</h3>
            <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
              Void <span className="font-medium text-gray-700 dark:text-gray-300">{soNumber}</span>? This will cancel the order.
            </p>
          </div>
        </div>
        <div>
...
export default VoidSalesOrderModal;
