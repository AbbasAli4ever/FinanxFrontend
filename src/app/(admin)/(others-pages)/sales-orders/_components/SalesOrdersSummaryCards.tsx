...
import type { SOSummary } from "@/types/salesOrders";

interface Props {
  summary: SOSummary | null;
  loading?: boolean;
}
...
const SalesOrdersSummaryCards: React.FC<Props> = ({ summary, loading }) => {
  if (loading) {
    return (
  ...
  const activeCount = sentCount + confirmedCount;
  const totalPending = summary?.totals.totalPending ?? 0;
  const partialCount = summary?.partial.count ?? 0;
  const partialAmount = (summary?.partial.amount ?? 0) + (summary?.sent.amount ?? 0) + (summary?.confirmed.amount ?? 0);
  const fulfilledCount = summary?.fulfilled.count ?? 0;
  const fulfilledAmount = summary?.totals.totalFulfilled ?? 0;
  ...
  const cards = [
    ...
    },
    {
      ...
      sub: formatCurrency(partialAmount),
      isOverdue: false,
    ...
    },
    {
      ...
      sub: formatCurrency(fulfilledAmount),
      isOverdue: false,
    ...
    },
    {
      ...
      sub: overdueCount > 0 ? formatCurrency(overdueAmount) + " at risk" : "All on track",
      isOverdue: true,
      iconBg: "bg-amber-100 dark:bg-amber-900/30",
      iconColor: "text-amber-600 dark:text-amber-400",
      icon: (
    ...
    },
    {
      ...
      sub: `${activeCount} order${activeCount !== 1 ? "s" : ""} in pipeline`,
      isOverdue: false,
      ...
      {cards.map((card) => (
        ...
        >
          ...
          <p className={`mt-0.5 text-xs ${
            card.isOverdue && overdueCount > 0 ? "text-amber-500" : "text-gray-400 dark:text-gray-500"
          }`}>{card.sub}</p>
        </div>
      ))}
    </div>
...
export default SalesOrdersSummaryCards;
