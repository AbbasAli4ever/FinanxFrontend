...
import type { CreateSORequest, CreateSOLineItemRequest, DiscountType } from "@/types/salesOrders";
...
const CreateSalesOrderModal: React.FC<Props> = ({
  ...
  const resetForm = () => {
    ...
    setShippingLine1(""); setShippingLine2(""); setShippingCity("");
    setShippingState(""); setShippingPostalCode(""); setShippingCountry("");
    setShowShipping(false); setLineItems([newRow()]); setError("");
  };
  ...
  const handleSubmit = async () => {
    ...
    if (!orderDate) { setError("Please enter an order date."); return; }
    ...
    try {
      ...
      await salesOrdersService.createSalesOrder(payload, token);
      ...
      <div className="flex flex-col gap-6">
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
          ...
          <div>
            ...
            <input type="text" value={referenceNumber} onChange={(e) => setReferenceNumber(e.target.value)} placeholder="e.g. PO-REF-001" className={inputCls} />
          </div>
          <div>
        ...
        <div className="rounded-xl border border-gray-200 dark:border-gray-800">
          <button type="button" onClick={() => setShowShipping(!showShipping)}
            ...
            <span className="flex items-center gap-2">
              ...
              Shipping Address <span className="ml-1 text-xs text-gray-400">(optional)</span>
            </span>
