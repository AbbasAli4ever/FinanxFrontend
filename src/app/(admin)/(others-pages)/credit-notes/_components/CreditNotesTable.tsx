...
import type { CreditNoteListItem, CreditNoteStatusInfo } from "@/types/creditNotes";
...
interface CreditNotesTableProps {
  ...
  onOpen: (cn: CreditNoteListItem) => void;
  onApply: (cn: CreditNoteListItem) => void;
  onRefund: (cn: CreditNoteListItem) => void;
  onVoid: (cn: CreditNoteListItem) => void;
}
...
function getStatusColor(statusInfo: CreditNoteStatusInfo): string {
  return statusInfo.color;
}

const CreditNotesTable: React.FC<CreditNotesTableProps> = ({
  creditNotes,
  onView,
  onEdit,
  onDelete,
  ...
  if (!creditNotes || creditNotes.length === 0) {
    return (
      <div className="rounded-2xl border border-dashed border-gray-300 bg-white/80 p-12 text-center dark:border-gray-700 dark:bg-gray-900/60">
        ...
        <p className="mt-3 text-sm font-medium text-gray-500 dark:text-gray-400">No credit notes found</p>
        <p className="mt-1 text-xs text-gray-400 dark:text-gray-500">Create a credit note to issue a refund or apply a credit to a customer invoice.</p>
      </div>
    );
  }
        ...
        <table className="min-w-full">
          <thead>
            <tr className="border-b border-gray-200 dark:border-gray-800">
              <th className="px-4 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-500 dark:text-gray-400">CN #</th>
              ...
              <th className="px-4 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-500 dark:text-gray-400">Invoice</th>
          ...
          <tbody className="divide-y divide-gray-100 dark:divide-gray-800">
            {creditNotes.map((cn) => (
              <tr
                key={cn.id}
                className="group transition hover:bg-gray-50/50 dark:hover:bg-white/[0.02]"
              >
                <td className="px-4 py-3">
                  <button
                    onClick={() => onView(cn)}
                    className="text-sm font-semibold text-brand-600 hover:text-brand-700 hover:underline dark:text-brand-400"
                  >
                    {cn.creditNoteNumber}
                  </button>
                  {cn.referenceNumber && (
                    <p className="text-[11px] text-gray-400">{cn.referenceNumber}</p>
                  )}
                </td>
                ...
                <td className="px-4 py-3 text-sm text-gray-500 dark:text-gray-400">
                  {cn.invoice ? (
                    <span className="text-xs text-brand-500">{cn.invoice.invoiceNumber}</span>
                  ) : (
                    <span className="text-xs text-gray-300 dark:text-gray-600">—</span>
                  )}
                </td>
                <td className="px-4 py-3 text-right text-sm font-medium tabular-nums text-gray-900 dark:text-white/90">
                ...
                <td className="px-4 py-3 text-center">
                  <Badge
                    variant="light"
                    color={getStatusBadgeColor(cn.status)}
                  >
                    {cn.statusInfo.label}
                  </Badge>
                </td>
                  ...
                  <div className="flex items-center justify-end gap-1">
                    ...
                    {cn.statusInfo.allowOpen && (
                      <button
                        ...
                        className="rounded-lg p-1.5 text-gray-400 hover:bg-success-50 hover:text-success-600 dark:hover:bg-success-900/20 dark:hover:text-success-400"
                      >
                        <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
                    ...
                    {cn.statusInfo.allowRefund && (
                      <button
                        onClick={() => onRefund(cn)}
                        title="Issue Refund"
                        className="rounded-lg p-1.5 text-gray-400 hover:bg-warning-50 hover:text-warning-600 dark:hover:bg-warning-900/20 dark:hover:text-warning-400"
                      >
                        <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
