...
import type { CreditNoteSummary } from "@/types/creditNotes";

interface CreditNotesSummaryCardsProps {
  summary: CreditNoteSummary | null;
}
...
const CreditNotesSummaryCards: React.FC<CreditNotesSummaryCardsProps> = ({ summary }) => {
  ...
  const cards = [
    ...
    {
      label: "Open",
      value: formatCurrency(summary.open.remainingCredit ?? summary.open.totalAmount),
    ...
    {
      label: "Partial",
      value: formatCurrency(summary.partiallyApplied.remainingCredit ?? summary.partiallyApplied.totalAmount),
      subtitle: `${summary.partiallyApplied.count} notes · remaining credit`,
    ...
    {
      label: "Outstanding",
      value: formatCurrency(summary.totalOutstandingCredit),
      subtitle: "total remaining credit",
  ...
  return (
    <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-5">
      {cards.map((card) => (
        <div
          key={card.label}
          className={`rounded-2xl border border-gray-200 p-4 shadow-sm dark:border-gray-800 ${card.bgClass}`}
        >
...
export default CreditNotesSummaryCards;
