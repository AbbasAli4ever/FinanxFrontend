...
interface RefundCreditNoteModalProps {
  isOpen: boolean;
...
const RefundCreditNoteModal: React.FC<RefundCreditNoteModalProps> = ({
  isOpen,
  creditNoteId,
  creditNoteNumber,
  remainingCredit,
  ...
  const { token } = useAuth();
  const [amount, setAmount] = useState<number | "">(remainingCredit);
  ...
  }, [isOpen, remainingCredit]);

  const handleSubmit = async () => {
    if (!token || !creditNoteId) return;
    if (!amount || Number(amount) <= 0) { setError("Please enter a valid amount."); return; }
    if (Number(amount) > remainingCredit) { setError(`Amount cannot exceed remaining credit of ${formatCurrency(remainingCredit)}.`); return; }
    if (!refundDate) { setError("Please enter a refund date."); return; }
    setLoading(true); setError("");
    try {
      await creditNotesService.refundCreditNote(creditNoteId, {
      ...
      <div className="flex flex-col gap-5">
        <div>
          ...
          <p className="mt-0.5 text-sm text-gray-500 dark:text-gray-400">
            Refund credit from <span className="font-semibold">{creditNoteNumber}</span> to customer
          </p>
        </div>
        ...
        <div className="flex items-center justify-between rounded-xl border border-brand-200 bg-brand-50/60 px-4 py-3 dark:border-brand-800 dark:bg-brand-900/10">
          <span className="text-sm font-medium text-brand-700 dark:text-brand-300">Available to Refund</span>
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
          ...
          <div>
            <Label>Refund Account (optional)</Label>
...
export default RefundCreditNoteModal;
